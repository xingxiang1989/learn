...
public class SkinThemeUtils {
    ...
    public static void updateStatusBarColor(Activity activity) {
        ...
        if (statusBarColorResId != 0) {
            int color = SkinResources.Companion.getInstance().getColor(statusBarColorResId);
            activity.getWindow().setStatusBarColor(color);
        } else {
            //获得 colorPrimaryDark
            int colorPrimaryDarkResId = getResId(activity, APPCOMPAT_COLOR_PRIMARY_DARK_ATTRS)[0];
            if (colorPrimaryDarkResId != 0) {
                int color = SkinResources.Companion.getInstance().getColor(colorPrimaryDarkResId);
                activity.getWindow().setStatusBarColor(color);
            }
        }
        if (navigationBarColor != 0) {
            ...
            int color = SkinResources.Companion.getInstance().getColor(navigationBarColor);
